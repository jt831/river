...
- 这是机制验证原型，除左上角调试时间外，尚无正式 UI、音频、关卡流程、存档、动画或测试代码。
...
`scripts/Deadzone.gd` 和 `scripts/Clearzone.gd` 是可复用的 `Area3D` 结果区域组件，共享 `GameResultZone` 的玩家分组筛选和触发逻辑。玩家进入 Deadzone 后显示全屏“游戏失败”，进入 Clearzone 后显示全屏“游戏成功”；场景会在结果画面出现时暂停，玩家点击任意位置后解除暂停并重载当前场景。默认玩家分组为 `player`，区域的位置、尺寸和碰撞层由宿主场景配置。
